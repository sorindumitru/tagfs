...
Based on the Nepomuk specification, Nepomuk-KDE is able to store in a RDF
(Resurse Description Framework) semantic data from desktop applications. For example
the Dolphin desktop manager is able to add simple tags to files or more complicated
comments. 
This solution is not limited to files metadata. Almost every application can use
the RDF store to add semantic metadata to theire objects. For example KMail can do it
for emails, Amarok can do it for music files, but it is used mostly for tagging files.
...
the file system tags apply; for a file system multiple entry points can be declared. This
allowed to keep the changes necessary for tagfs isolated so that the performance for the
normal case is not affected. This way the only difference from a vanilla kernel is that we
a string comparison for each entry point defined.
...
The metadata is stored into two hashtables. One for tags and one for files. 
...
file and afterwards, if necessary, performing the desired changes. The hooks are only called if
the file is inside one of the defined entry points.
\todo{hooks}
