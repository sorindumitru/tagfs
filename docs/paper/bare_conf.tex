...
TagFS implements a tag-based filesystem in Linux which offers support for 
...
a new one and various attempts to implement solutions have been made.

\subsection{Nepomuk-KDE}
...
\subsection{Other tag-fs}
existing kernel implementations for tag based filesystems.
...
Basically the filesystem hierarchy whill remain unchanged but files will be 
assigned tags (as it can be seen in \figref{img:hierarchy}).
\fig[scale=0.5]{figs/hierarchy.pdf}{img:hierarchy}{TagFS}

The TagFS architecture is presented in \figref{arch}. 
\fig[scale=0.5]{figs/archall.pdf}{arch}{TagFS architecture}

From a CLI TagFS associated opperations are issued, system calls are intercepted
in VFS and when required redirected through TagFS kernel module afterwards
being treated as regular commands by the file system.
    
\subsection{Storage}

\subsection{Tag handling}
